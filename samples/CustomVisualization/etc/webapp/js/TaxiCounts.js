...
// All Rights Reserved 
//
// ============ START OF TAXI COUNTS PAGE LOGIC ============
// This function gets called when the taxi counts page is loaded.
function taxi_counts_onload() {
	// Let us get the Taxi Counts metrics data periodically via a XHR timer.
	var url = window.location.href + "?request=SendTaxiCountMetrics&popup=true";
	loadData(url);	
	
	// Let us get the Taxi Counts hired trend data periodically via a XHR timer.
	url = window.location.href + "?request=SendTaxiCountTrend";
	loadTaxiCountHiredTrendData(url);
	
	// Let us get the Taxi Counts available trend data periodically via a XHR timer.
	url = window.location.href + "?request=SendTaxiCountTrend";
	loadTaxiCountAvailableTrendData(url);
}
...
function updateTheVisuals(responseText) {
	...
	response.map(function(item) {
		...
		// We will either push this JSON object in to the taxiCountsData array if it
		// doesn't exist in that array or simply update if it already exists in that array.
		var idx = -1;
		
		for(var i=0; i<taxiCountsData.length; i++) {
			if(taxiCountsData[i].ID == item.communityId) {
		...
		if(idx == -1) {
        	...
        	taxiCountsData[idx].Time = item.time;
        	taxiCountsData[idx].Name = item.communityName;
        	taxiCountsData[idx].District = item.districtName;
        	taxiCountsData[idx].Hired = item.hired;
        	taxiCountsData[idx].Available = item.available;
        }
    });
	...
	if (visualsAlreadyMade == false) {
		...
		// 3) Populate the Taxis Hired Trend dropdown box.
		populateTrendDropdownBox("community-for-hired-trend");
		
		// 4) Populate the Taxis Available Trend dropdown box.
		populateTrendDropdownBox("community-for-available-trend");
...
function createTable() {
	table = d3.select('#taxi-counts-table').append('table');
...
function createDonut1() {
	svg1 = d3.select("#taxis-hired-donut")
		.append("svg")
		.append("g")
...
function updateDonut1() {
	...
	for(var i=0; i<taxiCountsData.length; i++) {
		data.push({label : taxiCountsData[i].Name, 
			value: taxiCountsData[i].Hired,
			labelDisplayText : taxiCountsData[i].Name + ": " + taxiCountsData[i].Hired});
	}
...
function createDonut2() {
	svg2 = d3.select("#taxis-available-donut")
		.append("svg")
		.append("g")
...
function updateDonut2() {
	...
	for(var i=0; i<taxiCountsData.length; i++) {
		data.push({label : taxiCountsData[i].Name, 
			value: taxiCountsData[i].Available,
			labelDisplayText : taxiCountsData[i].Name + ": " + taxiCountsData[i].Available});
	}
...
// ======== START OF CREATING AND UPDATING THE TAXI COUNTS METRICS TREND ========
var communityIdForHiredTrend = "";
var communityIdForAvailableTrend = "";
...
function populateTrendDropdownBox(id) {
	...
	for(var i=0; i<taxiCountsData.length; i++) {
		communityNames.push(taxiCountsData[i].Name);
		// Store the community id in an associative array i.e. a Hashmap.
		communityIdsHashmap[taxiCountsData[i].Name] = taxiCountsData[i].ID;
	}
...
// selected in the taxis hired trend dropdown box.
function community_selection_hired_trend_onclick() {
	var select = document.getElementById("community-for-hired-trend");
...
function loadTaxiCountHiredTrendData(url) {	
	...
	// Refresh data every x seconds.
	var timeoutID = setTimeout(loadTaxiCountHiredTrendData, 5000, url);
}

// This function gets called whenever a community name is 
// selected in the taxis available trend dropdown box.
function community_selection_available_trend_onclick() {
	var select = document.getElementById("community-for-available-trend");
	// Get the current community name that is selected in the dropdown box.
	var currentValueSelected = select.value;
	
	// If there is no change in the selected community, simply return back.
	if(currentValueSelected == communityIdForAvailableTrend) {
		return;
	}
	
	// Let us set the new community name selected by the user which will be
	// picked up by the XHR timer method below.
	communityIdForAvailableTrend = currentValueSelected;
}

// This function is a timer driven one.
function loadTaxiCountAvailableTrendData(url) {	
	// Retreive data from the WebClientRequestHandler operator in the 
	// TaxiMetrics IBM Streams application.	
	//
	// Only when there is a community name selected by the user, we can perform this.
	if(communityIdForAvailableTrend != "") {
		//  Add the Community Name as a query string parameter.
		var newUrl = url + "&communityId=" + communityIdForAvailableTrend;
		// Construct HTTP request and send 
		var xhr = new XMLHttpRequest();
		xhr.open("GET", newUrl, true);
	...
	// Refresh data every x seconds.
	var timeoutID = setTimeout(loadTaxiCountAvailableTrendData, 5000, url);
}
...
function updateTheTrendGraph(responseText, trendType) {
	...
	if(trendType == 1) {
		id = "taxis-hired-trend";
		trend_line_class = "hired-trend-line";
	} else {
		id = "taxis-available-trend";
		trend_line_class = "available-trend-line";
	}
	...
	for(var i = 0; i < response.time.length; i++) {
		data.push(
			...
			{ 
				...
				"value" : (trendType == 1 ? response.hired[i] : response.available[i])
			}
        );	
	}
...
// ======== END OF CREATING AND UPDATING THE TAXI COUNTS METRICS TREND ======== 
// ============ END OF TAXI COUNTS PAGE LOGIC ============
