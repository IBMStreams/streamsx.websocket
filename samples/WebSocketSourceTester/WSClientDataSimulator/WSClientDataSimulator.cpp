...
3) Ensure that you have the OpenSSL installed on your Linux machine since we will need the OpenSSL crypto libraries in the compiler command used below.

4) Edit the Makefile in the same directory as this file and change the LD_LIBRARY_PATH to your C++ boost_1_69_0 lib directory in order to link to the Boost ASIO.

5) Edit the Makefile in the same directory as this file and change the WEBSOCKETPP_INSTALL_DIR to your websocketpp v0.8.1 installation directory.

6) Compile this test application by running the make command.

7) Once it is compiled, you can run it to send speech messages read from specific test data files to the WebSocketSourceTester
...
./wsclient.o https://b0513:443 /tmp/test1.wav bin 3500 5 8000 1000000
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 7) {
       ...
       std::cout << "Usage: ./wsclient.o  <WebSocket Server URL> <Binary or text file name> <bin or txt> <Data content block size in bytes to send at a time> <Delay in milliseconds between sending data content blocks> <Number of messages to send between verifying an ack from the server> <Loop count for repeated sending of the same file content>" << std::endl;
       std::cout << "Example: ./wsclient.o https://b0513:8443 /tmp/test1.wav bin 3500 5 8000 1000000" << std::endl;
       return(0);
    }
